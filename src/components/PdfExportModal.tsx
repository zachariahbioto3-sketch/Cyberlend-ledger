...
import { X, FileText, Download, CheckCircle2, Settings, User, BarChart2, Shield } from "lucide-react";
import { Loan, PortfolioMetrics } from "../types";
import {
  generatePortfolioSummaryPdf,
  generateLoanNoteStatement,
  generateDebtServiceReport,
...
type ReportType = "summary" | "loan_note" | "debt_service" | "standing";

export const PdfExportModal: React.FC<PdfExportModalProps> = ({
  isOpen, onClose, loans, metrics, theme: t,
}) => {
  ...
  const handleGeneratePdf = () => {
    ...
    try {
      if (reportType === "summary") {
        generatePortfolioSummaryPdf(loans, metrics, { organizationName, preparedBy, reportTitle, includePaidOff });
      } else if (reportType === "loan_note") {
        const loan = loans.find((l) => l.id === selectedLoanId);
        if (!loan) { alert("Please select a borrower."); setIsGenerating(false); return; }
        generateLoanNoteStatement(loan, organizationName, preparedBy);
      } else if (reportType === "debt_service") {
        generateDebtServiceReport(loans, organizationName, preparedBy);
      } else if (reportType === "standing") {
        generateStandingReport(loans, metrics, organizationName, preparedBy);
      }
  ...
  const reportOptions: { type: ReportType; icon: React.ReactNode; title: string; desc: string }[] = [
    { type: "summary",      icon: <BarChart2 className="w-4 h-4" />, title: "Portfolio Summary",      desc: "KPIs, loan schedule & full transaction ledger" },
    { type: "loan_note",    icon: <User className="w-4 h-4" />,      title: "Loan Note Statement",    desc: "Single borrower — account detail & payment history" },
    { type: "debt_service", icon: <FileText className="w-4 h-4" />,  title: "Debt Service Report",    desc: "All active loans — monthly obligations & coverage" },
    { type: "standing",     icon: <Shield className="w-4 h-4" />,    title: "Portfolio Standing",     desc: "Full health snapshot — overdue alerts & risk view" },
  ];
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
      ...
      <div className="relative w-full flex flex-col rounded-3xl overflow-hidden border"
        style={{ maxWidth: "74vw", minWidth: "74vw", maxHeight: "88vh", background: t.bgModal, borderColor: t.borderMid, boxShadow: "0 25px 60px rgba(0,0,0,0.5)" }}>
        ...
        <div className="flex items-center justify-between px-7 py-5 border-b shrink-0"
          ...
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="text-base font-bold tracking-widest" style={{ fontFamily: mono, color: t.text }}>EXPORT PDF REPORT</h3>
              <p className="text-xs mt-0.5" style={{ color: t.textMuted }}>Choose a report type and configure your output</p>
            </div>
          </div>
        ...
        <div className="flex-1 overflow-y-auto px-7 py-6 space-y-5">
          ...
          <div className="rounded-2xl p-5 border" style={{ background: t.bgCard, borderColor: t.border }}>
            <p className="text-[10px] font-bold uppercase tracking-widest mb-4" style={{ fontFamily: mono, color: t.textFaint }}>SELECT REPORT TYPE</p>
            <div className="grid grid-cols-2 gap-3">
              {reportOptions.map((opt) => (
                <button key={opt.type} onClick={() => { setReportType(opt.type); setHasDownloaded(false); }}
                  className="flex items-start gap-3 px-4 py-3 rounded-xl border text-left transition-all"
                  style={{
                    background: reportType === opt.type ? "rgba(91,124,250,0.12)" : t.bgActive,
                    borderColor: reportType === opt.type ? "#5b7cfa" : t.borderMid,
                  }}>
                  <span style={{ color: reportType === opt.type ? "#5b7cfa" : t.textMuted, marginTop: 2 }}>{opt.icon}</span>
                  <div>
                    <p className="text-xs font-bold" style={{ fontFamily: mono, color: reportType === opt.type ? "#5b7cfa" : t.text }}>{opt.title}</p>
          ...
          {/* BORROWER SELECTOR — only for Loan Note */}
          {reportType === "loan_note" && (
            <div className="rounded-2xl p-5 border" style={{ background: t.bgCard, borderColor: t.border }}>
              <p className="text-[10px] font-bold uppercase tracking-widest mb-3" style={{ fontFamily: mono, color: t.textFaint }}>SELECT BORROWER</p>
              {loans.length === 0 ? (
                <p className="text-xs" style={{ color: t.textMuted }}>No loans found.</p>
              ) : (
                <select value={selectedLoanId} onChange={(e) => setSelectedLoanId(e.target.value)}
                  ...
                  {loans.map((l) => (
                    <option key={l.id} value={l.id}>
                      {l.loanNumber} — {l.borrowerName} ({l.status})
                    </option>
                  ))}
                </select>
              )}
          ...
          {/* KPI SNAPSHOT */}
          <div className="rounded-2xl p-5 border" style={{ background: t.bgCard, borderColor: t.border }}>
            <p className="text-[10px] font-bold uppercase tracking-widest mb-4" style={{ fontFamily: mono, color: t.textFaint }}>PORTFOLIO SNAPSHOT</p>
              ...
              <>
                ...
                <div className="flex items-center gap-3 mt-4">
                  <input type="checkbox" id="include-paid" checked={includePaidOff}
                    onChange={(e) => setIncludePaidOff(e.target.checked)} className="w-4 h-4 rounded cursor-pointer" />
                  <label htmlFor="include-paid" className="text-xs cursor-pointer" style={{ color: t.textMuted }}>
                    Include completed & paid-off loans in report
                  </label>
                </div>
              </>
            )}
        ...
        <div className="flex items-center justify-between px-7 py-4 border-t shrink-0"
          style={{ background: t.bgCard, borderColor: t.border }}>
          <button onClick={onClose} className="px-5 py-2 rounded-xl text-sm font-bold transition-all border"
